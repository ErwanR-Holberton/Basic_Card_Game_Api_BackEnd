...
document.addEventListener("DOMContentLoaded", function () {
	...
	// Ajout des événements pour la sélection des catégories
	document.querySelectorAll(".topics-container button").forEach(button => {
...
function fetchData(category = null, page = null, limit = null) {
	...
	fetch(url)
		...
		.then(data => {
			...
			populatePagination(pagination1, data.pages, page || 1);
			populatePagination(pagination2, data.pages, page || 1);
	...
	articles.forEach(article => {
		...
		articleRow.innerHTML = `
			<span class="article-title">
				<a href="/forum/topic/${article.id}" class="article-link">${article.title}</a>
			</span>
